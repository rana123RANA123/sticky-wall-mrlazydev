...
export default function Register() {
  ...
  const createUserProfile = async (user) => {
    let { fullName, password, dob } = state
    ...
    try {
      await setDoc(doc(firestore, "register", uid), userData);
      // message.success("A new user has been created successfully")
          ...
          <div className="col">
            <div className="card p-3 p-md-4">
              ...
              <div className="col-12 mt-3 px-3">
                <p className='text-center'>Need an account? <Link to='/auth/login'><u className='text-dark fw-bold'>Login</u></Link></p>
              </div>
            </div>
          </div>
        </div>
